...
async def planner_register(request: Request, planner_data: planner_register):
    try:
        ...
        user = AuthorizationService.verify_session(request, db)
        ...
        planner_service.verify_planner(planner_data, user, db)
        planner_service.register_planner(user, planner_data, db)

        planner_service.register_planner_study_time(planner_data.date, user, db)
