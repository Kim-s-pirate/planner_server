...
class user_service:
    ...
    def edit_user(user_data: user_edit, current_userid: str, db):
        try:
            if user_data.userid != current_userid:
                user_service.edit_userid(user_data.userid, current_userid, db)
            if user_data.username != current_userid:
                user_service.edit_username(user_data.username, current_userid, db)
        except Exception as e:
            raise e

    def edit_user_by_id(user_data: user_edit, id: str, db):
        try:
            found_user = user_service.find_user_by_id(id, db)
            if found_user == None:
                raise UserNotFoundError
            if user_data.userid != found_user.userid and user_service.duplicate_userid(user_data.userid, db):
    ...
    def edit_username(new_username: str, userid: str, db):
        try:
            found_user = user_service.find_user_by_userid(userid, db)
            if found_user == None:
                raise UserNotFoundError
    ...
    def edit_userid(new_userid: str, userid: str, db):
        try:
            found_user = user_service.find_user_by_userid(userid, db)
            if found_user == None:
                raise UserNotFoundError
    ...
    def edit_email(new_email: str, userid: str, db):
        try:
            found_user = user_service.find_user_by_userid(userid, db)
            if found_user == None:
                raise UserNotFoundError
    ...
    def edit_password(new_password: str, userid: str, db):
        try:
            found_user = user_service.find_user_by_userid(userid, db)
            if found_user == None:
                raise UserNotFoundError
    ...
    def change_password(new_password: str, id:str, db):
        try:
            found_user = user_service.find_user_by_id(id, db)
            if found_user == None:
                raise UserNotFoundError
